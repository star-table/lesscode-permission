...
then
    Xmx="2048m"
elif [ "${SERVER_ENVIROMENT}"x = "crm2"x ]
then
    Xmx="512m"
elif [ "${SERVER_ENVIROMENT}"x = "crm"x ]
then
    Xmx="512m"
elif [ "${SERVER_ENVIROMENT}"x = "k8s-test"x ]
then
    Xmx="512m"
elif [ "${SERVER_ENVIROMENT}"x = "stag"x ]
then
    Xmx="1024m"
elif [ "${SERVER_ENVIROMENT}"x = "gray"x ]
then
    Xmx="4096m"
...
then
    ...
    echo "Environment variables SERVER_ENVIROMENT not found. 需定义环境变量 RUN_ENV：取值范围：local：本机；dev：开发环境；test：测试环境：stag|gray：预发布环境；prod：生产环境"
    exit -1
fi
...
MEM_OPTS=" -Xmx2048m -Xms2048m -Xmn${XMN_V}m -Xss1024k -XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=512m"
...
DEFAULT_JAR=$(find $APP_PATH/ -name *.jar)
